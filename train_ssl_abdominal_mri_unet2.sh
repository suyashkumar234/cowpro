#!/bin/bash
# train a UNet-based model to segment abdominal MRI (T2 fold of CHAOS challenge)
GPUID1=0
export CUDA_VISIBLE_DEVICES=$GPUID1

####### Shared configs
PROTO_GRID=8 # using 32 / 8 = 4, 4-by-4 prototype pooling window during training
CPT="myexperiments_UNet"
...
do
    ...
    do
    PREFIX="UNet_train_${DATASET}_lbgroup${LABEL_SETS}_scale_${SUPERPIX_SCALE}_vfold${EVAL_FOLD}"
    echo $PREFIX
    ...
    python training_unet2.py with \
    'modelname=unet_resnet34' \
    'usealign=True' \
    'optim_type=sgd' \
    'use_pretrained=True' \
    num_workers=$NWORKER \
    ...
    'tversky_params.tversky_beta=0.7' \
    'tversky_params.tversky_gamma=1.0'
    done
done